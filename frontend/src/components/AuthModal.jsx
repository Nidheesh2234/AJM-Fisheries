  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (activeTab === 'login') {
        ...
        if (!session) {
          setEmailConfirmNotice(true);
          setSuccess('Account created! Please confirm your email or turn off email confirmation in Supabase.');
        } else {
        ...
        <div className="modal-body">
          ...
          {emailConfirmNotice && (
            <div style={{
              ...
              <strong style={{ color: 'var(--color-gold)', display: 'block', marginBottom: '0.3rem' }}>
                💡 Instant Fix for "Email not confirmed":
              </strong>
              To allow immediate login without email verification:
              <ol style={{ paddingLeft: '1.2rem', marginTop: '0.3rem' }}>
                <li>Open <strong>Supabase Dashboard → Authentication → Providers → Email</strong></li>
                <li>Toggle <strong>"Confirm email"</strong> to <strong>OFF</strong> and click <strong>Save</strong>.</li>
              </ol>
            </div>
          )}
